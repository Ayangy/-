...
例：frontEnd/findExpertByLetter?letter=h
...
例：frontEnd/findOrganizationByName?query=安&type=1
...
| field       | String | 否    | ""   | 查询字段                                    |
| keyword     | String | 否    | ""   | 查询值                                     |
| textTypeId  | int    | 否    | 0    | 文章类型id                                  |
| parentId    | int    | 否    | 0    | 父级文章类型id                                |
| timeHorizon | int    | 否    | 0    | 时间范围：1(三天内),2(一周内),3(一月内),4(半年内),5(一年内) |
| sort        | int    | 否    | 0    | 排序：0(降序),1(升序)                          |

例：http://localhost:8080/frontEnd/findResultByCriteria?sort=1
